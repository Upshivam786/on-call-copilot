...
The Alembic environment reads the database URL from the environment when it is available.

Resolution

The correct host-side PostgreSQL port was 5434.

The migration command was executed with:
...
The application container should use:

postgres:5432

while host-side tools use:
...
The actual schema contained fields such as:
...
When debugging a database query failure, inspect the actual schema instead of guessing column names.
...
The actual document model uses:
...
The result showed Docker was already using port 8000.

The running containers were then inspected:

docker ps --format "table {{.ID}}\t{{.Names}}\t{{.Ports}}"

Another project was already exposing:
...
The API was then verified with:
...
was executed before the application source code had been copied into the image.

The Dockerfile initially contained:
...
The Dockerfile was changed so that the application package is available before installing the local package.

The resulting build completed successfully.

Lesson

Docker build order matters.

If pyproject.toml defines a local package such as:

packages = ["app"]

then that package must exist in the image when running:
...
However, the UI itself was running inside a Docker container.

Inside the UI container:

localhost

means the UI container itself, not the host and not the API container.

Correct Docker Networking

Docker Compose provides service-name DNS.

The API service is named:

api

Therefore the UI container should connect to:
...
localhost means different things depending on where the request originates.

This is one of the most common problems when containerizing multi-service applications.
...
and another change accidentally appended:
...
http://api:8000/api/v1/chat/sessions
Lesson
...
The retrieval service creates a BatchEmbedder.

The embedder's default path initializes:
...
The stored demo embeddings and the local embedding model also need to use compatible dimensions before the local fallback can be used against the existing pgvector data.

Lesson

Having a fallback implementation in a codebase does not automatically mean every execution path uses it.

Fallback logic must be wired into the actual dependency path.

10. Missing Postmortem Relationship
Observation

The database contains postmortem documents, but the seeded incidents currently have:
...
The timeline tool successfully retrieves incident details, but:

postmortem = None

when the incident has no linked postmortem.

Current Status

This does not prevent incident investigation or postmortem generation.

It simply means the current demo seed data does not establish a database relationship between those incidents and postmortem documents.
...
The generated postmortem displayed:
...
When using f-strings, distinguish between:
...
Always test individual layers independently.

A working UI can hide failures in lower-level components if the application has fallback behavior.
...
Useful commands used during troubleshooting include:
...
for host-to-container communication when the host port is mapped to 8002.

2. Configuration consistency

Keep database configuration consistent between:
...
is better than repeatedly trying random SQL column names.
...
A local embedding implementation existing in the repository is not enough.

The production retrieval path must explicitly select the correct embedder based on configuration.
